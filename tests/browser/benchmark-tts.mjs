...
if (process.env.BUDDY_SOFTWARE_WEBGL === "1") browserArgs.push("--use-angle=swiftshader");
...
page.on("requestfailed", (request) => {
  if (request.url().includes("/api/tts")) failedRequests.push({ url: request.url(), error: request.failure()?.errorText ?? null });
});
